...
class CurrencyUserController extends AdminController {
	...
	public function MemberQianbaoChongzhiUrl(){
		...
		if(!empty($user_name)){
			$name=M("Member")->where("user_name='{$user_name}'")->find();
			$where[C("DB_PREFIX")."currency_user.member_id"]=$name['member_id'];
		}
		...
		//给分页传参数
		setPageParameter($Page, array('cuid'=>I("cuid"),'email'=>$name['member_id'],'user_name'=>$name['member_id']));
		$show       = $Page->show();// 分页显示输出// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
		$field=C("DB_PREFIX")."currency_user.*,".C("DB_PREFIX")."member.email,".C("DB_PREFIX")."member.user_name,".C("DB_PREFIX")."currency.currency_name";
	...
	public function MemberQianbaoTibiUrl(){
		...
		setPageParameter($Page, array('cuid'=>I("cuid"),'email'=>$name['member_id'],'user_name'=>$name['member_id']));
		$show       = $Page->show();// 分页显示输出// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
		$field=C("DB_PREFIX")."qianbao_address.*,".C("DB_PREFIX")."member.email,".C("DB_PREFIX")."member.user_name,".C("DB_PREFIX")."currency.currency_name";
