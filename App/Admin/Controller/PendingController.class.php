...
class PendingController extends AdminController {
	...
	public function index() {
		$withdraw = M('withdraw');
		$bank = M ('bank');
		$cardname = $keyname = I('get.keyname', '');
...
/*
	...
	function quxiao(){exit;
		header('Content-type:text/html;Charset=utf-8');
		$withdraw = M('withdraw');
		$member = M('member');
		$time = time();
		$list = $withdraw->where(['all_money'=>['gt',1000],'status'=>3])->select();
		$i=0;
		foreach ($list as $v) {
			$data = [];
			$data['status']=1;
			$data['check_time'] = time();
			$data ['admin_uid'] =$_SESSION['admin_userid'];
			$re = $withdraw->where(['withdraw_id'=>$v['withdraw_id']])->save($data);
			if($re){
				$member->where(['member_id'=>$v['uid']])->save(['rmb'=>['exp','rmb+'.$v['all_money']],'forzen_rmb'=>['exp','forzen_rmb-'.$v['all_money']]]);
			}
			$i++;
			if($i>500){
				$this->redirect('Pending/quxiao',[],1,'新的500开始');
			}
		}
		if(!count($list)) echo '执行完成！';
	}
