...
class FinanceController extends AdminController{
	...
	public function index(){
        ...
        if(!empty($user_name)){
            $uid=M('Member')->where("user_name like '%{$user_name}%'")->find();
            $where[C("DB_PREFIX")."member.member_id"]=$uid['member_id'];
        }
        if(!empty($member_id)){
		...
		//显示日志
        $count = M('Finance')
        ->field(C("DB_PREFIX")."finance.*,".C("DB_PREFIX")."member.name as username,".C("DB_PREFIX")."member.user_name as user_name,".C("DB_PREFIX")."currency.currency_name,".C("DB_PREFIX")."finance_type.name as typename")
        ...
        //给分页传参数
        setPageParameter($Page, array('type_id'=>$type_id,'name'=>$name,'member_id'=>$member_id,'user_name'=>$user_name));
