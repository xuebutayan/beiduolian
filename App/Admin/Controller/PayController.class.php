...
class PayController extends AdminController {
    ...
    public function payByMan(){
            ...
            if(!empty($user_name)){
                $where[C("DB_PREFIX")."member.user_name"]=array('like',"%".$user_name."%");
            }
    	$count =  M('Pay')->where($where)->count();// 查询满足要求的总记录数
    	$Page  = new \Think\Page($count,20);// 实例化分页类 传入总记录数和每页显示的记录数(25)
    	//给分页传参数
    	setPageParameter($Page, array('status'=>$status,'member_name'=>$member_name,'user_name'=>$user_name));

    	$show       = $Page->show();// 分页显示输出

    	$list= M('Pay')
    	->field(C("DB_PREFIX")."pay.*,".C("DB_PREFIX")."member.email,".C("DB_PREFIX")."member.user_name")
