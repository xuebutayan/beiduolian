...
class PayController extends HomeController {
    ...
    public function rechargeByMan(){
        ...
        //查询累计充值
        $total = M('Pay')->where(['member_id'=>$this->member['member_id'],'status'=>1])->sum('money');
        if($data['money']>intval($total)){
            $arr['info']="充值金额大于累计限额{$config['chong_limit']}元";
            $arr['status']=0;
            $this->ajaxReturn($arr);
        }
